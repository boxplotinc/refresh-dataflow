...
- **v5.20**: Added API-verified concurrency guard. When the tracking row shows `Running` and the age is within the stale threshold, the concurrency guard now calls the Fabric API to confirm whether a real job is still active before blocking re-execution. If the API reports a terminal status (or the job does not exist at all — e.g., the notebook was killed before any trigger), the stale lock is repaired automatically and execution proceeds without requiring `force_run=True` or manual warehouse edits. Three verification paths are used in priority order: exact `job_instance_id` poll (most precise), CI/CD job-instance list endpoint (when no `job_instance_id` is stored), and regular dataflow transactions endpoint. If the API is unavailable or the dataflow type is unknown, the guard falls back conservatively to the original time-based stale check. All existing paths (`force_run=True`, stale-threshold bypass, CAS acquire, P1 pre-bucket repair flags) are unchanged.
- **v5.19**: Addressed all five April 2026 critique findings: (P1) Non-adhoc early-acquire lock repaired — three flags (`_main_row_acquired`, `_acquired_from_status`, `_entered_bucket_execution`) ensure the tracking row is restored to its prior terminal status when the run fails before any destructive bucket work begins, and the no-new-data branch now also restores the row instead of leaving it in Running. (P2) `_discover_dataflow_parameters` now follows `continuationToken`/`continuationUri` pagination across all pages before deciding support/non-support; early exit on first page that confirms both params. (P2) `_DeterministicPollError` class added; exact CI/CD job-instance poll raises it for HTTP 400/403/404; `wait_for_refresh_completion` counts consecutive hard errors and fails fast after 3 instead of burning the full timeout. (P3) `route_mode` allowlist validation added to `refresh_dataflow()` itself so direct callers get the same fail-fast contract as `execute_incremental_refresh()`. (P3) All stale `sys.exit` wording removed from notebook markdown and README.
